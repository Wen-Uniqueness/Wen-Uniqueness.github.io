...
## Deligne–Mumford compactication $\overline{M}_{g,n}$
...
3. Or, We say that an $\mathbf{FS}^{\mathrm{op}}$ module, $n \mapsto V_n$ is finitely generated in degree $\leq C$ if there is a finite list of classes ${v_i \in V_{d_i}}$ with $d_i \leq C$, such that every $V_n$ is spanned by classes of the form $f^* v_i$.



将$\{H_i(\overline{M}_{g,n})\}$包装成FS^op模，在这个意义上，我们将会阐释何为“有限生成性”（定理1.2的核心是“这个FS^op模有限生成” ）。

## 二叉树范畴BT^op与粘合作用

### 为什么需要BT^op？

FS^op不能直接作用在$\overline{M}_{g,n}$上（模空间的态射到对应满射的构筑不够自然）；

BT^op通过“粘合带标记的射影直线$\mathbb P^1$”（恰有3个标记点的射影直线只有一个同构类， 也就是树的枝桠对应到射影直线的构筑是直接的），诱导模空间的映射，进而传递到同调群，最终赋予$\{H_i(\overline{M}_{g,n})\}$ FS^op模结构。在最后的证明里，我们缩小n的方法就是不断的剪去这些“枝桠”。

### BT^op的定义与核心操作

- 定义：
    - 对象：自然数；
    - 态射：带标记的“二叉森林”（由二叉树组成，叶节点标记$[m]$、根节点标记$[n]$）；
    - 复合：将前一个森林的根与后一个的叶粘合，删除粘合点。
- 核心操作（粘合$P^1$）：
...
恰如三周前曾经讲过的表示稳定性（有兴趣的可以翻看上一个博客）[Limit, stability and representation: From Limit of homology groups to Representation Stability - Zijie Wen](https://wen-uniqueness.github.io/homological_stability-Homological-stability-and-representation-stability/)
...
### 稳定曲线 (Stable Curves)(前文提过，这里快速复述引出稳定图)
...
![稳定曲线和稳定图.png](http://Wen-Uniqueness.github.io/images/稳定曲线和稳定图.png)
...
## 将复杂结构转化为方便剪去的Y型枝桠：WDVV关系与同调消失性（选讲）
...
PROPOSITION 5.6. Let $g, n \geq 1$. Then $H_i^{\text{BM}}(\mathcal{M}_{g,n}) = 0$ for $i < n - 1$.

## 概念间的逻辑链（细讲时需强调）

$\overline{M}_{g,n}$（研究对象）变成了更加适合运算的$H_i^{\text{BM}}(\mathcal{M}_{g,n}) = 0$

然后通过稳定图来将BT^{op}上的作用对应到组合变化里

利用组合结论得到 $n> C$ 的时候，分支总是能通过WDVV变换变成“Y”型枝桠，然后再利用BT^op作用剪去，由此完成n的下降，最后得到有限生成性的证明。
